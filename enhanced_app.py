...
from utils.trade_history_parser import parse_trade_history_report
...
def create_enhanced_sidebar():
    ...
    with st.sidebar:
        ...
        report_type = st.radio(
            ...
            ["Backtester Report", "Trade Account History"],
            help="Choose between Strategy Tester results or real account trading history",
            key="report_type_selection"
        )
        
        # Show different info based on selection
        if report_type == "Backtester Report":
            st.info("📈 **Backtester Report**: Analyze MT5 Strategy Tester results and simulated trading performance.")
        else:
            st.info("💰 **Trade Account History**: Analyze real account trading history and live trading performance.")
        ...
        uploaded_file = st.file_uploader(
            ...
            type=['html', 'htm', 'csv'],
            help=f"Upload the HTML report for {report_type.lower()}. Large files (>10MB) will be automatically optimized.",
            key="file_uploader"
        )
        ...
        return uploaded_file, chart_theme.lower(), analysis_depth, risk_tolerance, feedback_level, risk_profile, confidence_level, report_type
...
def main():
    ...
    uploaded_file, chart_theme, analysis_depth, risk_tolerance, feedback_level, risk_profile, confidence_level, report_type = create_enhanced_sidebar()
    
    # Enhanced header with report type indicator
    if report_type == "Trade Account History":
        header_text = "📊 MT5 Professional Report Analyzer - Trade Account History"
        header_subtitle = "🏦 Analyzing Real Account Trading Performance"
    else:
        header_text = "📊 MT5 Professional Report Analyzer - Backtester Report"
        header_subtitle = "🧪 Analyzing Strategy Tester Results"
    
    st.markdown(f'<h1 class="main-header">{header_text}</h1>', unsafe_allow_html=True)
    st.markdown(f'<p style="text-align: center; color: #94a3b8; font-size: 1.1rem; margin-top: -2rem; margin-bottom: 2rem;">{header_subtitle}</p>', unsafe_allow_html=True)
    ...
    try:
        ...
        # Try multiple parsing strategies based on report type
        parsing_success = False
        
        if report_type == "Trade Account History":
            # Use trade history parser for real account data
            try:
                update_progress("Parsing trade account history...", 30)
                trades_df, summary_dict = parse_trade_history_report(uploaded_file, update_progress)
                
                if not trades_df.empty:
                    parsing_success = True
                    print(f"✅ Trade history parser success: {len(trades_df)} trades")
                
            except Exception as e:
                print(f"⚠️ DEBUG: Trade history parser failed: {str(e)}")
        
        else:
            # Use backtester parsers for Strategy Tester reports
