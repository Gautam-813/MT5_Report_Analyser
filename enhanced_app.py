...
def create_enhanced_sidebar():
    ...
    with st.sidebar:
        ...
        analysis_type = st.radio(
            ...
            ["Strategy Tester Report", "Live Trading History"],
            help="Select whether you're analyzing a strategy tester report or live trading history"
        )
        
        # File Upload Section
        st.markdown("#### 📁 **Upload Report**")
        if analysis_type == "Strategy Tester Report":
            st.info("💡 **Large File Support**: Files up to 50MB are supported. Processing time: ~30-60 seconds for files >10MB.")
            uploaded_file = st.file_uploader(
                "Choose MT5 Strategy Tester Report (HTML or CSV)",
                type=['html', 'htm', 'csv'],
            ...
            st.info("📈 **Live Trading Analysis**: Upload your MT5 account history report for live trading analysis.")
            uploaded_file = st.file_uploader(
                "Choose MT5 Live Trading History (HTML)",
                type=['html', 'htm'],
                help="Upload the HTML history report from your live MT5 account.",
        ...
        return uploaded_file, chart_theme.lower(), analysis_depth, risk_tolerance, feedback_level, risk_profile, confidence_level, analysis_type
...
def main():
    ...
    uploaded_file, chart_theme, analysis_depth, risk_tolerance, feedback_level, risk_profile, confidence_level, analysis_type = create_enhanced_sidebar()
    ...
    try:
        ...
        # Check if this is live trading analysis
        if analysis_type == "Live Trading History":
            update_progress("Processing live trading history...", 10)
            
            try:
                # Use live trading handler
                live_handler = process_live_report(uploaded_file)
                
                if live_handler is not None:
                    update_progress("Live trading data processed successfully!", 100)
                    ...
                    st.success("🎯 Live Trading Analysis Complete!")
                    
                    # Create live trading dashboard
                    live_handler.create_live_dashboard()
                    
                    # Add export functionality
                    st.subheader("📄 Export Live Trading Summary")
                    summary_report = live_handler.export_live_summary()
                    if summary_report:
                        st.download_button(
                            label="📥 Download Live Trading Report",
                            data=summary_report,
                            file_name=f"live_trading_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
                            mime="text/plain"
                        )
                    
                    return
                else:
                    st.error("❌ Failed to process live trading report. Please check the file format.")
                    return
                    
            except Exception as e:
                st.error(f"❌ Error processing live trading report: {str(e)}")
                return
        
        # Continue with strategy tester analysis for non-live reports
